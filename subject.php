...
<?php
$sql ="SELECT * FROM subjects WHERE subject_id    = '$sub_id' AND subjects.a_s = 1";
$result = query($sql);
confirm($result);
$row=fetch_array($result);
echo ucwords($row['sub_name']);
?>
        </div>
        </div> 
    </div>
...
<?php
    ...
    JOIN answers
    ON subjects.subject_id = answers.a_subject_id WHERE questions.q_no=answers.q_no AND questions.a_s = 1 AND answers.a_q = 1 AND answers.a_a = 1 AND answers.user_approve = 1 AND questions.user_approve = 1 AND subjects.subject_id   = '$sub_id'";
...
?>
    ...
    <!-- <div class="well" style="background-color:white;">
        <div class='alert alert-danger text-center'><strong>Question Not Found</strong> <br> We're sorry, trying to added this subject's questions.</div>
    </div> -->
